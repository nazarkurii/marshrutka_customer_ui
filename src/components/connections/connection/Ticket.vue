<template>
  <div class="flex w-full justify-center h-fit">
    <div
      @click="select"
      class="flex ticket cursor-default transition-all hover:translate-y-[-10px] min-h-[260px] hover:shadow-2xl shadow-black order-bg select-none duration-300 relative max-w-[730px] rounded-2xl w-full"
    >
      <p
        class="text-white font-bold text-lg rounded-2xl bg-green-500 size-fit py-[5px] px-[10px] absolute top-[-20px] left-[15px]"
      >
        {{ t('connections.result.cheapest') }}
      </p>
      <div class="flex flex-col h-full p-[15px] justify-center items-center gap-[20px] buy-part">
...
function ticketsLeft(sructure: ResponseSeat[][]): number {
  let left = 0
  for (let row of sructure) {
    for (let seat of row) {
      if (!seat.taken && seat.type != SeatType.Space && seat.type != SeatType.Table) {
        left++
      }
    }
  }
